...
class OcrDeskScanThread(QThread):
    target_found = pyqtSignal(dict)
    timeout_reached = pyqtSignal()

    def __init__(self, parent, target_text, region_params=None, timeout_seconds=10, min_confidence=0.5, ocr_engine=None, processing_interval=0.5):
    ...
    def run(self):
        ...
        x1, y1, x2, y2 = self.calculate_absolute_region(self.region_params)
        try:
            while self.running:
            ...
            print(f"OcrDeskScanThread: {e}")
        self.running = False

    def calculate_absolute_region(self, region_params, screen_width=1920, screen_height=1080):
        """
        将相对位置转换为绝对位置。

        :param region_params: 元组或列表，包含四个浮点数 (rx1, ry1, rx2, ry2)，范围 0.0-1.0
        :param screen_width: 屏幕宽度，默认 1920
        :param screen_height: 屏幕高度，默认 1080
        :return: 元组 (x1, y1, x2, y2)，绝对整数坐标
        """
        if len(region_params) != 4:
            raise ValueError("region_params 必须包含四个值: rx1, ry1, rx2, ry2")

        rx1, ry1, rx2, ry2 = region_params

        # 计算绝对坐标，并转换为整数（使用 int() 截断，或 round() 四舍五入，根据需求选择）
        x1 = int(rx1 * screen_width)
        y1 = int(ry1 * screen_height)
        x2 = int(rx2 * screen_width)
        y2 = int(ry2 * screen_height)

        # 确保 x1 < x2 和 y1 < y2，如果需要可以添加边界检查
        if x1 >= x2 or y1 >= y2:
            raise ValueError("计算后坐标无效: x1 >= x2 或 y1 >= y2")
